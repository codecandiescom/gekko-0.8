...
/** The language file directory for GeSHi
    @access private */
define('GESHI_LANG_ROOT', GESHI_ROOT . 'lang' . GESHI_DIR_SEPARATOR);
...
// functions for Gekko
function geshi_load_stylesheet($lang) {
	if (!cacheVar("geshi.style.$lang")) {
		$geshi = new GeSHI('', $lang);

		// adding stylesheet once
		pageSetMarkContent("STYLE", $geshi->get_stylesheet(false));
		cacheVar("geshi.style.$lang", true);

		unset($geshi);
	}
}

function geshi_highlight ($source, $lang, $lineno = false, $nocache = false) {

	$buff = "";

	$cache_key = cacheAssignKey($source, $lang);

	if ($nocache || !cacheRead($cache_key, $buff)) {
		//replace tabs in $source with spaces and then parse it as parameter to constructor
		$geshi = new GeSHi(str_replace("\t", "  ", $source), $lang);
		...
		if (!$nocache)
			cacheSave($cache_key, $buff = $geshi->error() ? nl2br(str_replace(" ", "&nbsp;", htmlspecialchars($source))) : $geshi->parse_code());

		unset($geshi);
	}

	//geshi_load_stylesheet($lang);
