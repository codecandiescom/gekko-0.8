...
class Crypt_Blowfish
{
    ...
    function encrypt($plainText)
    {
        if (!is_string($plainText)) {
            trigger_error('Plain text must be a string', E_USER_ERROR);
        }
    ...
    function decrypt($cipherText)
    {
        if (!is_string($cipherText)) {
            trigger_error('Chiper text must be a string', E_USER_ERROR);
        }
    ...
    {
        if (!is_string($key)) {
            trigger_error('Key must be a string', E_USER_ERROR);
        }

        $len = strlen($key);

        if ($len > 56 || $len == 0) {
            trigger_error('Key must be less than 56 characters and non-zero. Supplied key length: ' . $len, E_USER_ERROR);
        }
        ...
        require_once GEKKO_SOURCE_DIR.'lib/third_party/crypt_blowfish/key.php';
        $this->_init();
